...
Dumbridge feature-detects those methods. A patched binding configures the
requested environment or explicit URL proxy. Stock `@number0/iroh@1.0.0`
continues to fail with `BridgeProxyUnsupportedError`, before binding a socket.
Proxy URLs and native error strings are not included in public typed errors.
