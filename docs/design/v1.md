...
Constructed once from a user-supplied path. It owns canonicalization, path containment, symlink policy, safe relative paths, and sanitized display paths. It opens a fixed `/workspace` shell view and a pull-shaped `inspect` / `list` / `scan` view. The host path, Node handles, and `OverlayFs` policy never leave the module.
...
pull:     manifest? -> (file-start -> file-chunk* -> file-end)* -> (complete | pull-error)
```
