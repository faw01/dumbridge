...
Constructed once from a user-supplied path. It owns canonicalization, path containment, symlink policy, safe relative paths, and sanitized display paths. It opens a fixed `/workspace` read view for consumers; the host path and `OverlayFs` policy never leave the module.
