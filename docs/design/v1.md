...
| **Bridge link** | A secret, versioned bearer value containing a transport kind, opaque locator, and random capability. |
| **Remote read shell** | Just Bash running against a copy-on-write view of the served root. It is not the Mac's real Bash. |
| **Run** | Execute one Bash-shaped query against the remote read shell and stream its output. |
...
Running `dumbridge` with no verb shows help. There is no `setup`, `connect`, `open`, `grant`, `audit`, `revoke`, `send`, or `receive` command in v1.
...
4. prints a redacted summary plus a `DUMBRIDGE_LINK` value;
5. serves requests until Ctrl-C;
6. closes the endpoint and forgets the capability on exit.
...
Writes made by the script go only to a fresh in-memory overlay and disappear after the request. `rm`, `mv`, `chmod`, and `>` therefore cannot mutate the served root. Network access, Python, and JavaScript execution are disabled.
...
Constructed once from a user-supplied path. It owns canonicalization, path containment, symlink policy, safe relative paths, and sanitized display paths. No downstream module receives an arbitrary host path.
...
Owns endpoint lifetime, accept/connect, deadlines, close semantics, and backpressure. The Iroh adapter privately owns ALPN, direct versus relay paths, HTTP proxy configuration, and the current JavaScript/native byte conversion required by the Node-API binding. The public interface deals in bounded byte sessions, not Iroh classes.

The first production implementation uses `@number0/iroh`. An in-memory implementation supports protocol tests. A future HTTPS queue, if hosted proxies reject Iroh relay WebSockets, implements this interface without changing product commands, filesystem policy, or `dumbridge/1`.
...
pull:     manifest -> file-start -> file-chunk* -> file-end -> complete
```
...
run(link, script): Effect<RunResult, RunError>
pull(link, remotePath, destination): Effect<PullResult, PullError>
```
...
Without one opaque bridge link, v1 would need at least one of:
...
3. build cloud links with a relay-only endpoint address;
...
- leakage of the bridge link through normal logs or error messages.

### We do not defend against

- a cloud agent or process that has the bridge link intentionally reading anything below the served root;
- a bridge link copied from the environment while the bridge is active;
- a compromised local user account reading the served root directly;
- a malicious local process rebinding the served path and restoring its original identity entirely within one filesystem operation;
...
- Verify the served root identity before and after every host-backed filesystem operation and discard the result if it changed.
...
- Link redaction in structured logs, errors, telemetry, process titles, and test fixtures.
- Public security reporting must use GitHub private vulnerability reports, not public issues.

Node and Just Bash do not expose directory-handle-relative `openat`-style I/O. The per-operation identity guard therefore prevents a result from escaping whenever a rebound is observed, but cannot detect a rebound and restoration completed entirely between its two checks.

### Cloud credential reality

Codex environment variables exist during the agent phase, while Codex secrets are removed after setup. The CLI therefore needs `DUMBRIDGE_LINK` as an environment variable, not a Codex secret. Cursor secrets are injected as agent environment variables, which supplies the same bearer semantics. No design can let an agent invoke a bearer-authenticated CLI while making the bearer unknowable to every process acting as that agent.

The mitigation is scope: serve only the needed root, start immediately before the task, stop immediately after it, and keep v1 read-only. Serving the entire GitHub directory intentionally grants the link holder read access to every `.env` and uncommitted file below it.
...
Start as one real application package inside a Bun workspace. Turborepo is useful immediately as the single task entry point; do not invent empty `core`, `protocol`, or transport packages before they have independent consumers.
...
│   ├── design/v1.md
│   ├── research/
│   └── adr/                  # only after decisions are confirmed
├── AGENTS.md
├── README.md
├── biome.jsonc
├── bun.lock
...
Keep `BridgeTransport` as an internal deep-module boundary in `apps/dumbridge` initially. If a hosted HTTPS implementation or separately built Rust adapter becomes real, extract the implementations into workspace packages then; Turborepo already gives that move a home.
...
| Linux x64 musl | Support and test. |
...
Install Dumbridge in the environment setup, store the short-lived link as a scoped Cursor secret, and allow the Iroh relay host. Cursor documents outbound network modes but does not promise UDP or WebSocket behavior, so the real hosted-agent test remains mandatory.
...
- provide `DUMBRIDGE_LINK` as an environment variable;
...
13. `SECURITY.md` and GitHub private vulnerability reporting cover capability leakage, path escape, sandbox escape, and unauthorized reads.
...
- [Runtime and CLI decision](../research/runtime-and-cli.md)
- [Iroh ecosystem findings](../research/iroh-ecosystem.md)
- [Cloud connectivity](../research/cloud-connectivity.md)
- [Scaffolding research](../research/scaffolding.md)
