...
The bridge stays attached to the terminal by default; Ctrl-C is revocation. V1 does not install a daemon: `serve --detach <root>` runs the same bridge without a terminal (several may run at once, at most one per served root), `serve --stop [<root>]` ends one and revokes its key, and `serve --status` lists each active detached serve with its served root, pid, start time, and key expiry, pruning stale records as it lists.

`serve` also owns the connection-path policy through two mutually exclusive flags (see “Connection-path reporting and forcing” below): `--direct-only` mints a key with no relay fallback, and `--relay-only` mints a key whose initial dial goes through the relay, best effort only.
...
pull:     reject | pull-error | (manifest -> (file-start -> file-chunk* -> file-end)* -> (complete | pull-error))
```
...
### Connection-path reporting and forcing

Whether a session travels peer-to-peer or through a relay is observable and steerable, because release gate 2's forced-relay proof and gate 9's throughput benchmarks are uninterpretable without it.

After `run` or `pull` connects, the client prints one stderr line naming the path selected at connect time — `dumbridge: connected directly`, `dumbridge: connected via relay`, or `dumbridge: connected (path unknown)` when no snapshot is observable. The line is stderr-only, so piped stdout stays exactly the script's or pull's own output. It is a connect-time snapshot, not a live watcher: iroh may upgrade a relayed session to a direct path mid-connection, and that upgrade goes unreported in v1. The value crosses the transport seam as the iroh-agnostic `ConnectionPath` (`"direct" | "relay" | "unknown"`) on every `BridgeSession`; no iroh types leak past `BridgeTransport`.

The forcing flags live on `serve` because the constraint propagates through the minted key: the dialing endpoint derives its relay mode from the locator, so a `serve --direct-only` key carries direct addresses only and every client dial is direct-only without client-side coordination. A direct-only dial that cannot holepunch fails fast with the branded `BridgeDirectConnectError` ("No viable network path to the bridge: the direct connection failed (this network may block UDP) and the bridge locator allows no relay fallback.") instead of hanging or silently degrading; the client does not retry it.

A failed dial against a relay-carrying key is classified at the transport seam, because only the iroh adapter can watch the relay link while the dial runs: a dial that fails with the relay link up is reported as the bridge not answering (serve stopped or the machine offline), and a dial that fails with the link down names the relay host that could not be reached — the egress-policy case, where the fix is allowlisting that exact host (iroh's public relays live under `relay.n0.iroh.link`), not restarting serve or rotating the key. The classification crosses the seam as the iroh-agnostic `BridgeDialError` with a `reason` literal and the relay host; no iroh types leak past `BridgeTransport`. When the unusable-proxy fallback preceded a dial that then failed, the client appends the proxy as the likely cause — only after a real network failure, never pre-network. `--log-level debug` on `run` or `pull` logs the dial sequence (paths attempted, relay used, outcomes) on stderr, never the bridge key, the opaque locator ticket, or proxy credentials. A proxy-capable client is the one exception: it dials relay-only because UDP holepunching cannot traverse an HTTP proxy, so a direct-only key fails there immediately with the no-relay locator error rather than spending a connect deadline on a dial that cannot succeed. When a proxy variable is set but the installed binding cannot route through it — the published `@number0/iroh` omits the proxy builder methods, and Claude Code cloud's "Full Network" is this shape while still offering a working direct path — the client does not commit to the unusable proxy: it prints one stderr warning (never the proxy URL, which may carry credentials) and falls back to the ordinary direct-capable dial, leaving relay policy to the locator in the key. A proxied environment with no working direct or relay route (Codex cloud) then fails as a genuine connection failure instead of the pre-network `BridgeProxyUnsupportedError` dead-end. `serve --relay-only` strips direct addresses from the locator, which constrains only the initial dial: the established session may still upgrade to a direct path, because a hard relay-only mode needs Rust `clear_ip_transports()`, which the current Node binding does not expose. The flag is documented as best effort and never claimed as a guarantee; a true forced-relay test mode waits on iroh-ffi patch work. ADR 0005 records this decision.
...
│       ├── src/
│       │   ├── cli.ts        # Effect CLI adapter and entry point
│       │   ├── key-source.ts # bridge key resolution: --key-file, stdin, env
│       │   └── bridge/       # bridge process composition: server, client,
│       │                     # channel glue, detached serve control
│       ├── test/
│       ├── package.json
│       └── tsconfig.json
├── packages/
...
│   └── typescript-config/    # shared tsconfig base every package and app extends
├── docs/
│   ├── design/v1.md
│   └── adr/
...
Package internal layout follows one proportionate rule: a package is a single `src/index.ts` until it outgrows it; a grown package keeps `index.ts` as the export surface over sibling modules (wire, served-root, pull-transfer); an adapter gets its own subdirectory and export path (`bridge-transport/src/iroh`, exported as `./iroh`). Every package and app tsconfig extends `@dumbridge/typescript-config/base.json` with explicit `include`/`exclude`, and the root `tsconfig.json` is a thin relative extend of the same base.
...
- allow the exact relay host(s), because `iroh.link` is not in the common-dependencies preset: iroh's public relays are `aps1-1.relay.n0.iroh.link`, `euc1-1.relay.n0.iroh.link`, `use1-1.relay.n0.iroh.link`, and `usw1-1.relay.n0.iroh.link`, all over HTTPS (TCP 443);
