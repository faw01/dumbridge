...
When connection failures persist, run `dumbridge doctor`: it needs no bridge key and opens no session, prints one self-descriptive check per line — DNS resolution of the iroh relay hosts, UDP egress, relay reachability on port 443, HTTP(S) proxy capability, and extra CA trust for TLS-intercepting proxies — and exits non-zero when any check fails. Use it to identify proxy-only or UDP-blocked environments before asking the user to verify that the bridge process is running.
...
- `The bridge did not answer: the relay is reachable, so dumbridge serve has likely stopped or the local machine is offline.` - the network path is fine; ask the user to start `dumbridge serve` again and provide the new key.
- `The relay at <host> could not be reached: ...` - this network's egress policy likely blocks the named relay host. Ask for HTTPS (TCP 443) to that exact host to be allowed - dumbridge uses iroh's public relays under `relay.n0.iroh.link` - rather than restarting serve or rotating the key.
- `No viable network path to the bridge: the direct connection failed (this network may block UDP) and the bridge locator allows no relay fallback.` - the key was minted with `serve --direct-only` and this network cannot reach the bridge peer-to-peer. Ask the user to serve without `--direct-only`, or check that `dumbridge serve` is still running.
- `... if this network allows egress only through that proxy, no direct or relay path can succeed.` - appended to a connect failure after the unusable-proxy warning: the environment forces a proxy the installed iroh binding cannot use, and the fallback dial also failed. The network configuration, not the serve process, is the likely cause.
- `... names an extra CA certificate ... connecting without it` - this environment points at an extra CA certificate (for a TLS-intercepting proxy) that could not be used; the connection proceeds without it. Harmless unless the proxy intercepts TLS, in which case the following relay failure is explained by the missing CA trust.
- `The bridge process is unreachable.` - the connection failed before a cause could be observed. Ask the user to check that `dumbridge serve` is still running.
- When a connection failure is unclear, rerun with `--log-level debug` (for example `dumbridge run --log-level debug 'true'`): it logs the dial sequence - paths attempted, relay used, outcomes - on stderr without exposing the bridge key.
