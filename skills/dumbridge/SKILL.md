...
When connection failures persist, run `dumbridge doctor`: it needs no bridge key and opens no session, prints one self-descriptive check per line — DNS resolution of the iroh relay hosts, UDP egress, relay reachability on port 443, and HTTP(S) proxy capability — and exits non-zero when any check fails. Use it to tell a proxy-only or UDP-blocked environment apart from a stopped bridge.
